...
def download_cpt_codes_as_csv(request):
    if request.method == "POST":
        cpt_codes_mappings = master_data_models.CPTCodesMapping.objects.filter(facility = request.user.profile.facility)
        model_fields = master_data_models.CPTCodesMapping._meta.fields + master_data_models.CPTCodesMapping._meta.many_to_many
        field_names = [field.name for field in model_fields]
        ...
        # Write a first row with header information
        #
        # json_object = cpt_codes_mappings.first().payload_object
        #
        #
        # for key in jsonObject:
        #     fields.append(key)
